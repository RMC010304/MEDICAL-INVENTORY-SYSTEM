...
namespace MIS_PROJECT
{
    public partial class UserControl10 : UserControl
    {
        ...
        {
            int id = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["Item_No"].FormattedValue.ToString());
            SqlConnection con = new SqlConnection("Data Source=localhost;Initial Catalog=MIS;User ID=sa;Password=12345678");
            con.Open();
            SqlCommand com = new SqlCommand("DELETE dbo.Disinfectants2 WHERE Item_No= '" + id + "' ", con);
            com.ExecuteNonQuery();
